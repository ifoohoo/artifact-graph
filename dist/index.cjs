...
var import_node_child_process2, import_node_fs3, import_promises4, import_node_path7, import_node_util2, execFileAsync2, KNOWN_COMMANDS;
...
async function loadConfig(root) {
  const configPath = (0, import_node_path11.join)(root, "artifact-graph.config.yaml");
  let parsed = {};
  try {
...
function validateE2eConfig(e2e) {
  ...
  if (e2e.runners !== void 0) {
    ...
    for (const runner of e2e.runners) {
      ...
      if ((0, import_node_path11.isAbsolute)(runner.root)) {
...
async function scanArtifacts(root, schema) {
  ...
  for (const [type, definition] of artifactTypeEntriesBySpecificity(config)) {
    ...
    for (const file of files) {
      ...
      const raw = await (0, import_promises7.readFile)((0, import_node_path11.join)(root, file), "utf-8");
  ...
  const absoluteRoot = (0, import_node_path11.isAbsolute)(root) ? root : (0, import_node_path11.resolve)(root);
...
async function validateScenarioPrdLinkIndex(root, graph) {
  ...
  let raw = "";
  try {
    raw = await (0, import_promises7.readFile)((0, import_node_path11.join)(root, indexPath), "utf-8");
  } catch (error) {
    if (error.code === "ENOENT") {
      return [];
    }
...
async function writeGraphCache(root, graph) {
  const cacheDir = (0, import_node_path11.join)(root, ".artifact-graph");
  await (0, import_promises7.mkdir)(cacheDir, { recursive: true });
  await (0, import_promises7.writeFile)((0, import_node_path11.join)(cacheDir, "index.json"), `${JSON.stringify(graph, null, 2)}
`);
  const { default: Database } = await import("better-sqlite3");
  const db = new Database((0, import_node_path11.join)(cacheDir, "graph.sqlite"));
  try {
    db.exec(`
...
function parseGenericMarkdown(type, path, raw, schema) {
  const diagnostics = [];
  const ext = (0, import_node_path11.extname)(path).toLowerCase();
...
function isTestFile(filePath) {
  ...
  const name = (0, import_node_path11.basename)(normalized);
...
function parseE2eTest(path, raw) {
  ...
  const nodes = [];
  const edges = [];
  const batch = String(data.test_batch ?? (0, import_node_path11.basename)(path, (0, import_node_path11.extname)(path))).trim();
...
function parseE2eRegistry(path, raw) {
  ...
  return {
    nodes: [{ type: "e2e_registry", code: (0, import_node_path11.basename)(path, (0, import_node_path11.extname)(path)), title: "E2E Test Registry", path, line: 1, attrs: data }],
    edges: []
  };
}
...
async function validateExecutableTraceability(root, config) {
  const issues = [];
  const schema = config ?? await loadConfig(root);
  const e2eDir = (0, import_node_path11.join)(root, "artifacts", "tests", "e2e");
  let e2eFiles;
  try {
    e2eFiles = (await (0, import_promises7.readdir)(e2eDir)).filter((name) => /^test-.*\.md$/.test(name)).map((name) => (0, import_node_path11.join)(e2eDir, name));
  } catch {
    return [];
  }
  ...
  for (const filePath of e2eFiles) {
    ...
    const relPath = (0, import_node_path11.relative)(root, filePath).split("\\").join("/");
    const parsed = (0, import_gray_matter.default)(raw);
    const data = parsed.data;
    const batch = String(data.test_batch ?? (0, import_node_path11.basename)(filePath, (0, import_node_path11.extname)(filePath))).trim();
    const lines = raw.split(/\r?\n/);
    const tcStarts = [];
    lines.forEach((line, index) => {
  ...
  for (const specFile of specFiles) {
    const fullSpecPath = (0, import_node_path11.join)(root, specFile);
    let content;
    try {
  ...
  for (const [tcKey, { ref, chainType, path, line }] of mdToRef) {
    ...
    for (const entry of refEntries) {
      ...
      if (entry.testId) {
        let content;
        try {
          content = await (0, import_promises7.readFile)((0, import_node_path11.join)(root, normalizedRefFile), "utf-8");
        } catch {
          continue;
        }
...
async function computeE2eCoverageStats(graph, root, thresholds = {}) {
  ...
  for (const filePath of e2eFiles) {
    ...
    const parsed = (0, import_gray_matter.default)(raw);
    const batch = String(parsed.data.test_batch ?? (0, import_node_path11.basename)(filePath, (0, import_node_path11.extname)(filePath))).trim();
  ...
  for (const node of e2eNodes) {
    ...
    for (const entry of parseExecutableRefLines(execRef)) {
      ...
      if (!normalized || !(0, import_node_fs5.existsSync)((0, import_node_path11.join)(root, normalized))) continue;
  ...
  for (const node of featureNodes) {
    const acs = parseAcceptanceCriteria(await (0, import_promises7.readFile)((0, import_node_path11.join)(root, node.path), "utf-8"));
...
async function generateE2eRegistry(root, opts) {
  ...
  for (const file of files) {
    const filePath = (0, import_node_path11.join)(e2eDir, file);
    const raw = await (0, import_promises7.readFile)(filePath, "utf-8");
    const parsed = (0, import_gray_matter.default)(raw);
    const data = parsed.data;
    const batch = String(data.test_batch ?? (0, import_node_path11.basename)(file, (0, import_node_path11.extname)(file))).trim();
...
async function validatePartialRustEvidence(tcFields, tcKey, root, allFiles) {
  ...
  for (const ref of rustRefs) {
    ...
    const fullPath = (0, import_node_path11.join)(root, normalizedPath);
    let content;
    try {
...
function resolveExecutableRefFile(refFile, allFiles) {
  ...
  if (!normalized || (0, import_node_path11.isAbsolute)(refFile) || normalized.split("/").includes("..")) {
    return void 0;
  }
...
async function hasMarkdownTc(tcKey, e2eDir) {
  ...
  const filePath = (0, import_node_path11.join)(e2eDir, `${batch}.md`);
  try {
    const raw = await (0, import_promises7.readFile)(filePath, "utf-8");
...
function normalizeDesignCode(value) {
  ...
  return (0, import_node_path11.basename)(raw, (0, import_node_path11.extname)(raw));
}
...
function resolveArtifactContext(graph, opts) {
  ...
  if (universalBaseline) {
    ...
    if (root) {
      for (const ap of ALWAYS_PRESENT_ITEMS) {
        const fullPath = (0, import_node_path11.join)(root, ap.path);
        let stat;
        try {
...
var import_gray_matter, import_js_yaml, import_node_fs5, import_promises7, import_node_path11, TARGET_ARTIFACT_TYPES, NON_TARGET_ROLES, DEFAULT_SCHEMA, EMPTY_RELATION_FIELD_VALUES, VALID_TC_STATUSES, VALID_CHAIN_TYPES, DEPRECATED_CHAIN_TYPE_ALIASES, CONTEXT_CATEGORIES, TIER_ORDER;
